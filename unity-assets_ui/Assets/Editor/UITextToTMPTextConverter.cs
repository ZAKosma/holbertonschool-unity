...
public class TextToTextMeshPro : Editor
{
    ...
    static void ConvertTextToTextMeshPro(GameObject target)
    {
        ...
        TMP_FontAsset tmpFontAsset = FindTMPFontAsset(uiText.font);
    ...
    static TMP_FontAsset FindTMPFontAsset(Font font)
    {
        if (font == null)
        {
            return null;
        ...
        foreach (string tmpGuid in guids)
        {
            string tmpPath = AssetDatabase.GUIDToAssetPath(tmpGuid);
            Debug.Log("Checking TMP_FontAsset path: " + tmpPath);
            ...
            TMP_FontAsset tmpFontAsset = AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(tmpPath);

            if (tmpFontAsset != null)
            {
                Debug.Log("TMP_FontAsset name: " + tmpFontAsset.name);
                Debug.Log("Unity UI Text font name: " + font.name);

                if (tmpFontAsset != null && tmpFontAsset.name.Equals(font.name))
                {
                    Debug.Log("Found TMP_FontAsset: " + tmpFontAsset.name);
                    return tmpFontAsset;
                }
            }
        }

        Debug.Log("No matching TMP_FontAsset found for font: " + font.name);
        return null;
    }
